...
BOOL CMvSmartCamCtrlDemoDlg::OnInitDialog()
{
    ...
    int     nRet = MV_OK;
    CString cstrInfo;
    m_SRHandle = SR_CreateHandle();
    if (0 == m_SRHandle)
    {
        cstrInfo.Format("创建渲染句柄失败!nRet = [%#x]", nRet);
        MessageBox(cstrInfo);
        return 0;
    }

    nRet = SR_Init(m_SRHandle, SR_PF_MONO8, SR_ENGINE_D3D, 0);
    if (0 == m_SRHandle)
    {
        cstrInfo.Format("渲染初始化失败!nRet = [%#x]", nRet);
        MessageBox(cstrInfo);
        return 0;
    }

    nRet = SR_AddSubPort(m_SRHandle, 0, SR_ET_REDBLUE_3D);
    if (SR_RETURN_OK != nRet)
    {
        cstrInfo.Format("渲染增加端口失败!nRet = [%#x]", nRet);
        MessageBox(cstrInfo);
        return 0 ;
    }
...
void CMvSmartCamCtrlDemoDlg::DisplayOneFrame()
{
    ...
    while (m_bStartJob)
    {
        ...
        if (MV_OK == nRet)
        {
            if (PixelType_Sc_Mono8 == m_stImageOutInfo.enPixelType)
            {
                nRet = SR_UpdateFrameData(m_SRHandle, m_pData, m_stImageOutInfo.nWidth, m_stImageOutInfo.nHeight, 0);
                if (SR_RETURN_OK != nRet)
                {
                    cstrInfo.Format("渲染更新Data失败!nRet = [%#x]", nRet);
                    MessageBox(cstrInfo);
                    return ;
                }
                nRet = SR_Display(m_SRHandle, 0, GetDlgItem(IDC_STATIC_DISPLAY)->GetSafeHwnd());
                if (SR_RETURN_OK != nRet)
                {
                    cstrInfo.Format("渲染更新Data失败!nRet = [%#x]", nRet);
                    MessageBox(cstrInfo);
...
void CMvSmartCamCtrlDemoDlg::OnBnClickedCheckSoftware()
{
    ...
    {
        nRet = MV_SC_SetEnumerationValue(m_handle, "TriggerSource", SC_TRIGGER_MODE_ON);   //选择软触发
        if (MV_OK != nRet)
        {
    ...
    {
        nRet = MV_SC_SetEnumerationValue(m_handle, "TriggerSource", SC_TRIGGER_MODE_OFF);
        if (MV_OK != nRet)
        {
